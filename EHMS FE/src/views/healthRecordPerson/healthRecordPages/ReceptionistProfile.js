...
function ReceptionistProfile() {
  ...
  const updateProfile = async (data) => {
    ...
    try {
      if (editable === true) {
        setLoading(true);
        const _data = await ReceptionistServices.getReceptionistProfileUpdate(data, config);
        setLoading(false);
        Swal.fire({
          icon: "success",
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        setLoading(true);
        const _data = await ReceptionistServices.getReceptionistProfile(config);
        setResponse(_data.data);
        setLoading(false);
        console.log(_data.data);
       setImageFile(_data.imageFile);
